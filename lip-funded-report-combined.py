...
def connection():
    ...
    return funded_encompass_latest, funded_point_latest

@task
def merge_reports(funded_encompass_latest, funded_point_latest):
    ...
    columns_to_keep = [
        ...
        "LIEN_POSITION","FUNDED_DATE", "LOAN_TERM"
    ]

    funded_point_latest = funded_point_latest[columns_to_keep]


    funded_point_latest.set_index(["LOAN_NUMBER","TBD","NOTES","BORROWER_FIRST_NAME","BORROWER_LAST_NAME","STREET","STATE","FUNDED_DATE","CLOSING_DATE","LOCK_EXPIRATION","LOAN_AMOUNT","NOTE_RATE","POINTS","WIRE_AMOUNT","LO_BROKER",\
    ...
    funded_encompass_latest.rename(columns={'TOTAL_LOAN_AMOUNT': 'LOAN_AMOUNT', 'TITLE_ORDERED_DATE': 'TITLE_ORDERED', 'EST_CLOSING_DATE':'CLOSING_DATE', 'SUBJECT_PROPERTY_ADDRESS':'STREET', 'LGC_FUND_SOURCE':'FUND','LOAN_PROGRAM':'LOAN_TYPE',\
    ...
    columns_to_keep = ["LOAN_NUMBER","BORROWER_FIRST_NAME","BORROWER_LAST_NAME", "NOTE_RATE", "LOAN_AMOUNT","CLOSING_DATE","FUNDED_DATE","STREET","LIEN_POSITION", "FUND", "LOAN_TYPE","LO_BROKER", "TITLE_ORDERED", "TITLE_RECEIVED","LOCK_EXPIRATION","POINTS","APPRAISAL_ORDERED","APPRAISAL_DUE",\
    "STATE", "APPROVED","CLEAR_TO_CLOSE","WIRE_AMOUNT","APPRAISAL_EXPIRATION","CLOSING_LIKELIHOOD","OCCUPANCY","LOAN_TERM"]

    funded_encompass_latest = funded_encompass_latest[columns_to_keep]

    funded_encompass_latest.set_index(["LOAN_NUMBER","BORROWER_FIRST_NAME","BORROWER_LAST_NAME", "NOTE_RATE", "LOAN_AMOUNT","CLOSING_DATE","FUNDED_DATE","STREET","LIEN_POSITION","FUND", "LOAN_TYPE","LO_BROKER","TITLE_ORDERED","TITLE_RECEIVED","LOCK_EXPIRATION","POINTS","APPRAISAL_ORDERED","APPRAISAL_DUE",\
    "STATE", "APPROVED","CLEAR_TO_CLOSE","WIRE_AMOUNT", "APPRAISAL_EXPIRATION","OCCUPANCY","LOAN_TERM"])

    funded_encompass_latest

    concatenated_table = pd.concat([funded_point_latest, funded_encompass_latest], axis=0, ignore_index=True)
    ...
    columns_to_convert = ['CLOSING_DATE','FUNDED_DATE','BP_DOC_REVIEW','APPRAISAL_ORDERED','APPRAISAL_DUE','APPRAISAL_EXPIRATION','TITLE_ORDERED','TITLE_RECEIVED','DOCS_OUT','SIGNING_APPOINTMENT']
    ...
    column_order = ['LOAN_NUMBER', 'TBD', 'NOTES', 'BORROWER_FIRST_NAME',
        'BORROWER_LAST_NAME', 'STREET', 'STATE', 'FUNDED_DATE', 'CLOSING_DATE','CLOSING_LIKELIHOOD',
...
def email_csv(concatenated_table):
    ...
    postmark.emails.send(
        ##list of emails 'nickw@legacyg.com'##
        From='legacy-info@rook.capital',
        To=['dustinh@legacyg.com','nickw@legacyg.com','ColtonF@legacyg.com','JanaA@legacyg.com'],
        Subject=f"Updated Funded Lip Report - {current_date}",
        HtmlBody=f"Please find the updated FUNDED LIP report attached as of {current_date}.",
        Attachments=[
            {
                "Name": f"LIP FUNDED - {current_date}.csv",
                "Content": encoded_csv,
...
def get_dynamic_filename():
  ...
  dynamic_filename = f"lip_funded_combined_{today_date}.csv"
...
def upload_csv_to_bucket_with_block(gcs_bucket_block_name: str, df: pd.DataFrame, destination_blob_name: str):
  try:
    ...
    gcs_bucket = GcsBucket.load("gcs-lip-funded-block")
...
def LIP_FUNDED_REPORT() -> None:
    funded_encompass_latest, funded_point_latest = connection()
    concatenated_table = merge_reports(funded_encompass_latest, funded_point_latest)

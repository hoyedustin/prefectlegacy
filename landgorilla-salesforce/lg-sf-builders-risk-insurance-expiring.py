...
def pull_accounting_all_active_loans_report(latest_id_value, api_token):
    ...
    df["Builder's Risk Insurance Expiration"] = pd.to_datetime(df["Builder's Risk Insurance Expiration"])
    
 
    today = pd.Timestamp.now().normalize()
 
    df["Days Until Builder's Insurance Due"] = (df["Builder's Risk Insurance Expiration"] - today).dt.days

    logger.info(df["Days Until Builder's Insurance Due"])
 
    need_to_extend_builders = df[df["Days Until Builder's Insurance Due"] == 30]
 
    logger.info(need_to_extend_builders)
   
    return need_to_extend_builders
 
 
@task
def create_sf_case(need_to_extend_builders):
 
    logger = get_run_logger()
 
    if need_to_extend_builders.empty:
        logger.info("No loans have builder's risk insurance expiring in 30 days. Exiting the script.")
        return
   
 
    for index, row in need_to_extend_builders.iterrows():
        if row["Days Until Builder's Insurance Due"] == 30:
            case_data = {
                'Subject': f"Builder's Risk Insurance expiring in 30 days for loan number: {row['Loan Number']}",
                'Description': row['Loan Number'],
                'RecordTypeId': '0128c000001eYpiAAE',
                'OwnerId': '00G8c0000068j8mEAA',
                'Reason' : "Builder's risk Insurance: Expires in 30 days",
...
@flow
def lg_to_sf_builders_risk_insurance_case_create() -> None:
    api_token = authenticate()
    latest_id_value = get_most_recent_id_value(api_token)
    need_to_extend_builders = pull_accounting_all_active_loans_report(latest_id_value, api_token)
    create_sf_case(need_to_extend_builders)
